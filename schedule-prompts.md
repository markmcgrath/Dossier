...
> You are running the morning LinkedIn lead pulse check. This is a lightweight, fast check — not a full triage. **Step 1: Load state file.** Read `Dossier/.lead-pulse-state.json` to get `seen_thread_ids` (Gmail thread IDs from prior runs) and `seen_fingerprints` (entries of the form `{fingerprint, thread_id, first_seen, company_hint}`). If the file or either field is missing, initialize empty arrays. **Step 2: Search Gmail for new recruiter signals** using these queries in parallel: LinkedIn message notifications (newer than 1 day), LinkedIn InMail replies, LinkedIn job alerts (only if role looks like a match per `profile.md`), self-forwarded leads, and cold recruiter outreach not from job boards. **Step 3: Deduplicate.** For each candidate thread: (a) if its thread ID is in `seen_thread_ids`, discard; (b) build a fingerprint `{recruiter_key}|{role_key}` (see Fingerprint Rules below) and check `seen_fingerprints` for an exact match or a fuzzy match (role keys sharing 3 of 4 tokens). If matched AND `first_seen` is within the last 21 days, suppress the lead silently — add the thread ID to `seen_thread_ids` but do not write the lead. If matched but older than 21 days, re-surface and note in the leads-file entry: *"Previously seen [date] — re-surfacing after 21-day grace period."* **Step 4: If zero new non-suppressed threads — stop here. Exit silently.** No file written. **Step 5: For each surviving thread, extract** recruiter name, company, role, comp signal, and message type, plus the fingerprint components (see Fingerprint Rules). **Step 6: Quick profile filter** — flag as ✓ MATCH, ✗ SKIP, or ? UNCLEAR based on the target roles and roles-to-avoid in `profile.md`. **Step 7: Cross-check pipeline** — is this company already tracked? Check `Dossier/config.md` for `notion.enabled`; query Notion or scan `Dossier/evals/` accordingly. **Step 8: Write the leads file** to `Dossier/leads-[YYYY-MM-DD]-am.md` only if at least one ✓ MATCH or ? UNCLEAR survived. If any leads were fingerprint-suppressed AND at least one non-suppressed lead exists, append a `## Suppressed (fingerprint duplicates)` footer with the suppressed count. If all threads were ✗ SKIP or suppressed — silent exit. **Step 9: Update state file.** Append new thread IDs to `seen_thread_ids` and new entries `{fingerprint, thread_id, first_seen, company_hint}` to `seen_fingerprints`. Trim both arrays to the last 200 entries, oldest-first. **Hard rules:** Read-only on Gmail. No Notion writes. No drafts. No calendar events.
>
> **Fingerprint Rules** — `recruiter_key`: prefer sender email (lowercased); fall back to the LinkedIn profile slug extracted from `linkedin.com/comm/in/[slug]` links in the body; last resort, signature name normalized to lowercase-hyphens. `role_key`: role title lowercased; drop stop words `a|an|the|senior|lead|remote|opening|opportunity|role`; replace spaces with hyphens; strip punctuation; keep the first 4 significant tokens. Fingerprint format: `{recruiter_key}|{role_key}`.
...
> You are running the afternoon LinkedIn lead pulse check. Identical logic to the morning pulse but use `newer_than:8h` to focus on the mid-day window. **Step 1: Load state file** `Dossier/.lead-pulse-state.json` — both `seen_thread_ids` and `seen_fingerprints` (initialize empty arrays if missing). **Step 2: Search Gmail** with the same recruiter signal queries as the AM pulse, using `newer_than:8h`. **Step 3: Deduplicate.** For each candidate: (a) discard if the thread ID is in `seen_thread_ids`; (b) build fingerprint `{recruiter_key}|{role_key}` (see AM-pulse Fingerprint Rules) and match against `seen_fingerprints` (exact match, or fuzzy match on 3-of-4 role-key tokens). Suppress silently if matched within the last 21 days (add the thread ID to `seen_thread_ids` but don't write). If matched but older than 21 days, re-surface with a note: *"Previously seen [date] — re-surfacing after 21-day grace period."* **Step 4: If zero new non-suppressed threads — stop and exit silently.** **Step 5: For each surviving thread, extract** recruiter name, company, role, comp signal, message type, plus fingerprint components. **Step 6: Quick profile filter** against `profile.md` — ✓ MATCH / ✗ SKIP / ? UNCLEAR. **Step 7: Cross-check pipeline** for each company. Read `config.md` to determine data source (Notion or `Dossier/evals/`). **Step 8: Write the leads file** to `Dossier/leads-[YYYY-MM-DD]-pm.md` only if at least one ✓ MATCH or ? UNCLEAR survived. If any leads were fingerprint-suppressed AND at least one non-suppressed lead exists, append a `## Suppressed (fingerprint duplicates)` footer with the count. Silent exit if all are ✗ SKIP or suppressed. **Step 9: Update state file.** Append new thread IDs and fingerprint entries; trim both to the last 200 entries, oldest-first. **Hard rules:** Read-only on Gmail. No Notion writes. No drafts. No calendar events.
