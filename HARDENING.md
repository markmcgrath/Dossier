...
- The pinned timestamp is `2026-01-01T00:00:00Z` for every entry. The `built_at` field in `skill/manifest.json` is also pinned to this value (not a wall-clock timestamp). The `commit` and `version` fields in the manifest are the only volatile elements — they reflect HEAD at pack time, so they necessarily change with each commit even when `skill/` source is unchanged. The content-match guard in `build_skill.sh` therefore compares every ZIP entry **except `skill/manifest.json`**: bundles are *content-identical* across rebuilds at the same source state, just not byte-identical. The guard exits 3 if any non-manifest entry drifts (extra/missing files or content mismatch), catching contributors who edit `skill/` without re-running the packer; recovery is `cp dist/dossier-*.skill dossier.skill && git commit && git push && retag` (delete the prior tag/release first if the tag was already pushed).
...
- Solo-maintainer public repos where every merge can reasonably go through a PR.
- Projects where content drift matters (AI skills, prompt packages, docs-heavy repos).
- Projects with PII or secret concerns that benefit from a committed scanner.
- Projects where the maintainer wants the hardening itself to be auditable evidence of work quality.

### Where it's overkill

- Quick prototypes, personal scripts, and hobby code without CI. The ruleset + required-status-check plumbing is overhead without contributors.
- Repos with no secrets, no PII, and no external contributors.

### Where it's underkill
...
Drop-in patterns that travel cleanly to any project of any size:

- **Workflow token defaults: `read`, no self-approve.** See §3.
- **Workflow-level `permissions: contents: read`** on any workflow that doesn't need to write. Least privilege below the repo default.
- **Deterministic artifact packing.** §9: sorted file order, pinned timestamps, DEFLATE. Minimizes diff noise and makes rebuilds reproducible.
- **Gitignored local PII patterns file** with a committed template. §5. Works for any repo where contributors have maintainer-specific identifiers they want to guard against leaking.
- **`tests/fixtures/` path skip** in any content scanner. Fixtures are synthetic look-alike data by design.
- **User-layer / system-layer separation** with a documented data contract. §8. Applies any time an upstream distribution could overwrite a downstream user's customization.
- **Ruleset-cycle discipline for admin force-pushes.** §1, §9. Keeps the force-push visible in the ruleset audit log rather than hidden behind a bypass-actor carve-out.
- **Required-status-checks in strict mode.** Forces the PR branch up-to-date with base before merge, so CI actually runs against the code that will land.

### What's Dossier-specific — don't copy blindly

- Specific ruleset rule bundle, PII regex list, allowlist substrings, and test assertions. They guard Dossier's specific contracts; your project has different contracts.
- `required_approving_review_count: 0` is a solo-maintainer accommodation, not a recommendation. Default to ≥ 1 for anything else.
- Empty `bypass_actors` — workable at solo scale because the ruleset-cycle friction is tolerable. At team scale, consider either a scoped bypass actor or a separate "break-glass" ruleset with broader alerting on its use.
- Actions SHA-pinning disabled is only defensible because this repo uses first-party `actions/*` exclusively. **Turn it on the moment you add a third-party action**, even a reputable one.
- The PII scanner's eight built-in patterns are a floor. Your local `pii_patterns.local.txt` is where the real protection lives — fill it in with your own names, alternate emails, employer identifiers, and internal tokens.

### Suggested adaptation order

If you're bootstrapping a similar repo from scratch, roughly this order minimizes rework:

1. Add `.github/workflows/ci.yml` with `permissions: contents: read` and at least one `test` job.
2. Add `.github/scripts/pii_scan.py` plus `pii_patterns.template.txt` and `.gitignore` entries for the local-patterns file.
3. Wire both into CI as required jobs.
4. Create a ruleset on `main` with the five rules from §1, adjusting `required_approving_review_count` to your team size.
5. Configure Actions to `selected` with the narrowest allowlist your workflows actually need.
6. Lock down workflow-token defaults (Settings → Actions → Workflow permissions → Read).
7. Enable Dependabot alerts, security updates, and version updates.
8. After the repo is public: secret scanning, push protection, CodeQL default setup.
9. Write your own `HARDENING.md` capturing what you did and why.
