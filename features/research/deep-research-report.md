...
A key dependency implied by `dashboard.md` is the GitHub community plugin “Dataview” for Obsidian-style vault querying: it treats a vault “as a database,” extracting data from YAML frontmatter and inline fields, and supports both a query language and a JavaScript API. 
...
This is functionally similar (at the workflow level) to more mature “career ops” systems that turn AI agents into a job-search command center, producing standardized artifacts and supporting batch evaluation and dashboards. For example, `santifer/career-ops` advertises a multi-mode system with dashboards, evaluation at scale, and legal disclaimers. 
...
- Legal/compliance: **not addressed** in supplied files; this is important because platform ToS (LinkedIn, Indeed) explicitly restrict automation. 
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `santifer/career-ops` | MIT | JavaScript, Go | CLI + Go dashboard + skill modes | Multi-mode job ops, dashboards, evaluation, artifact outputs | 33.3k stars / 6.6k forks / 122 commits | High (docs + disclaimers) | Yes (`.github`) | Evidence of test scripts (`test-all.mjs`) | Has `SECURITY.md` + ToS disclaimers | Local CLI; project-defined tooling | |
| `Gsync/jobsync` (JobSync) | MIT | TypeScript | Self-hosted web app (Next.js) | Tracks applications, matching, AI resume review, analytics | 516 stars / 88 forks / 431 commits | High (quick start + provider config) | Yes (`.github/workflows`) | Yes (`__tests__`, `e2e`, jest config) | Some security practices implied (auth secret guidance) | Docker Compose, self-hosted | |
| `JustAJobApp/jobseeker-analytics` | MIT | TypeScript, Python | Web app + Gmail ingestion | Automated tracking via inbox, dashboarding | 184 stars / 81 forks / 2,434 commits | High (docs/use cases + deployment) | Likely (has `.github`) | Not confirmed from root listing | Explicit security orientation (“open for audit”) + `SECURITY.md` | Docker Compose files present | |
| `1291pravin/job-hunt-ai` | MIT | TypeScript, Vue | Local-first scraper + matcher (Playwright + SQLite) | Job discovery, matching rubric, local data store | 2 stars / 0 forks / 8 commits | Medium (clear stack + workflow) | Unclear | Not found | Emphasizes local privacy, but tool risk from scraping/logins | Local; suggests Docker/VPS constraints | |
| `reggiechan74/JobOps` | MIT indicated, but README contains conflicting language | HTML/CSS/JS/Shell | Claude-code style command arsenal + artifact folders | Strong overlap: scoring rubrics, file naming conventions, OSINT reports, multi-step resume build | 16 stars / 4 forks / 93 commits | Medium–High (large README, workflow docs) | Yes (`.github/workflows`) | Not established from listing | License inconsistency in docs suggests governance gap | Local-first; optional tooling (e.g., Playwright MCP) | |
| `feder-cr/jobs_applier_ai_agent_aihawk` | AGPL-3.0 | Python | Automation agent (browser automation) | Automation-heavy job applying | 29.7k stars / 4.5k forks | Medium (popular, but risks) | Unknown | Unknown | High compliance/ToS risk; automation posture | Local automation | |
| `ammarlakis/obsidian-system-job-tracker` | MIT | JavaScript | Obsidian vault template + scripts | Very close in spirit: vault, frontmatter, dashboards, scripts | 6 stars / 0 forks / 4 commits | High for its scope (install + plugins + scripts) | Unknown | Scripted automation exists | Security not addressed; relies on plugin trust | Local vault | |
| `DrLeucine/obsidian-job-dashboard` | No license found | (Vault / Markdown) | Obsidian dashboard draft (Dataview + Mermaid) | Dashboarding + tracking concepts | 67 stars / 1 fork / 5 commits | Medium (explicit draft; minimal docs) | Unknown | N/A | License absent (reuse risk) | Local vault | |
| `infews/job_search_in_obsidian` | No license found | (Vault / Markdown) | Obsidian notebook/process repository | Process guidance; vault-based workflow | 44 stars / 4 forks / 5 commits | Medium (narrative process docs) | Unknown | N/A | License absent (reuse risk) | Local vault | |
| `xitanggg/open-resume` (OpenResume) | AGPL-3.0 | TypeScript | Browser-first resume builder/parser | Strong overlap with Tailored CV intent | 8.5k stars / 975 forks | High (docs, docker, features) | Unclear | Jest config present | Emphasizes “runs locally” privacy | Local dev + Docker | |

### What the ecosystem suggests for Dossier

Across these projects, the projects that feel “top-tier” share common traits:

- **A working reference implementation** (not just conventions): JobSync and JustAJobApp are deployable systems with clear quick-start paths and ongoing releases. 
- **A security and legal/compliance stance**: career-ops explicitly warns about ToS compliance and model hallucinations and includes security policy files. 
- **Automation + trust boundaries**: Obsidian-based templates work well when they include scripts, dashboards, and clear plugin requirements; however, they still need guidance around plugin trust and safe JavaScript query use. 
...
|---|---|---|---|
| NIST AI RMF 1.0 | Standard (PDF) | Frames AI risk as socio-technical; defines 4 core functions (GOVERN, MAP, MEASURE, MANAGE) and lists trustworthiness characteristics incl. security, privacy, fairness. | Add a lightweight “AI risk governance” section: define intended use, failure modes (hallucinations), human oversight, and update cadence for prompts/rubrics. |
| NIST Generative AI Profile (AI 600-1) | Standard (PDF) | A profile for GenAI risk management; emphasizes governance, content provenance, pre-deployment testing, and incident disclosure; explicitly treats GenAI risk as needing tailored actions. | Add provenance/traceability fields to artifacts (model used, date, sources), plus an incident/bug reporting workflow for bad outputs or data leaks. |
| OWASP Top 10 for LLM Applications | Standard | Enumerates common LLM app risks (prompt injection, insecure output handling, supply chain, etc.). | Use OWASP LLM Top 10 as a checklist for the Notion/Gmail/Calendar integration design; add guardrails for tool invocation and output handling. |
| Liu et al., “Prompt Injection attack against LLM-integrated Applications” | Academic (arXiv) | Explores real prompt injection risks in LLM-integrated apps. | Treat external content (emails, job postings, scraped pages) as untrusted; insert a “sanitization + instruction separation” step before tool actions. |
| Greshake et al., “Not what you’ve signed up for” (Indirect Prompt Injection) | Academic (arXiv) | Shows how instructions hidden in retrieved content can hijack LLM behavior. | Critical for Mode 9 (inbox) and Mode 2/4 (web research): require a “tool-action confirmation gate” when content comes from outside the vault. |
| Bender et al., “On the Dangers of Stochastic Parrots” | Academic | Highlights risks: bias, environmental costs, opacity, and data harms; urges careful use and documentation. | Add “known limitations” and bias cautions to scoring outputs; avoid overstating fit; document uncertainty and assumptions. |
| Raghavan et al., “Mitigating Bias in Algorithmic Hiring” | Academic (PDF) | Explains that “bias mitigation” claims in hiring tech are complex; data choices and targets matter; fairness is not a one-line fix. | Ensure Dossier does not encode harmful assumptions (e.g., “culture fit” as proxy bias); provide anti-bias guidance in rubric usage. |
| Jones & Teevan, “Personal Information Management” (publisher page) | Academic / foundational | PIM frames how people store/organize/retrieve personal info across formats and roles. | Validates the file-first approach; suggests adding retrieval affordances (indexes, consistent metadata, dashboards) as first-order UX. |
| OpenSSF SLSA | Standard/guidance | Supply-chain security framework to prevent tampering and improve integrity. | If Dossier becomes a code project, adopt SLSA-aligned release practices: signed releases, pinned deps, CI provenance. |
| SPDX SBOM definition | Standard | SBOM = inventory describing package composition, provenance, licensing, and known issues. | Add SBOM generation for any published CLI/plugins; declare licenses and dependency policies early. |
...
- **No canonical frontmatter schema**: without explicit required/optional keys, vault queries become fragile. This is a common failure mode of Dataview-driven systems, because Dataview relies on consistent metadata extraction from YAML/inline fields. 
...
- **No pipeline state machine**: the scoring rubric produces grades, but there is no formal state model (e.g., `identified → evaluated → outreached → applied → interviewing → offer → closed`). Comparable systems often encode states explicitly to power dashboards and automation. (Example: job-hunt-ai defines a job workflow graph and statuses.) 

### Scalability and performance gaps

- **Batch processing undefined**: the skill is written for interactive use; there is no pattern for bulk evaluation or for caching repeated company research. Mature systems advertise batch evaluation and dashboards at scale. 
...
- **Onboarding is incomplete**: users need a “starter vault” (example data + dashboards + templates) and a guided setup path. Projects like JobSync provide a clear Docker quick start and UI onboarding. 
- **No “one-command create artifact” tooling**: Obsidian-based trackers often include scripts and QuickAdd forms to create/update entries quickly. 
...
- **No provenance tracking**: NIST GenAI profile emphasizes governance and incident disclosure; that implies tracking model outputs and sources. 
...
- **No CI/CD**: top-tier projects typically ship with automated checks and release pipelines. For open-source security posture, tools like Scorecard and SLSA-oriented practices are common anchors. 
...
- **No contributor docs**: missing `CONTRIBUTING`, code of conduct, governance, and security policy. Mature comparables include these files, and explicitly cite transparency as a security strategy when handling sensitive user data. 
...
- **Direct prompt injection**: attacker-crafted input tries to override system instructions. 
- **Indirect prompt injection**: malicious instructions embedded in retrieved data (emails, web pages, documents) hijack the agent, especially when tools are available. 
...
- LinkedIn prohibits third-party software such as crawlers/bots/extensions that scrape or automate activity. 
- Indeed prohibits automation/scripting/bots to automate the Indeed Apply process outside official vendors/tooling. 
...
Notably, `career-ops` explicitly instructs users to comply with third-party ToS and warns against spamming employers. That type of disclaimer is a practical baseline for this space. 
...
- DataviewJS runs with the same level of access as other plugins and can rewrite/create/delete files and make network calls; only use scripts you trust. 
...
- Adopt a secure SDLC baseline (NIST SSDF) for development practices. 
- Use SLSA to harden build provenance and reduce tampering risk. 
- Publish SBOMs (SPDX) for releases so users and downstream projects can assess composition and risk. 
- Run automated project posture checks (OpenSSF Scorecard). 
...
- **Security posture**: publish SBOMs for releases and achieve a “passing” OpenSSF Scorecard baseline (with explicit remediation for failing checks). 
- **Docs quality**: onboarding time for a new user to first evaluation ≤15 minutes (measured via a scripted walkthrough).
- **Compliance safety**: default modes do not violate major platform ToS (LinkedIn/Indeed) and require manual confirmation for any external automation. 
...
- The **strongest differentiator** of the Dossier skill is its **structured scoring rubric** and **artifact-first governance** (produce evidence, not just advice). This aligns with both personal information management research and AI risk governance principles: documentation and traceability reduce error impact and improve user trust. 
- The **largest operational risk** is not model quality—it is **unsafe agency**: any workflow that reads untrusted external content (emails/webpages) and can trigger actions (messages, scheduling, logging) is exposed to indirect prompt injection and must implement strict tool-action boundaries. 
- The **largest legal/compliance risk** is automating activity on platforms that explicitly prohibit it (LinkedIn automation, Indeed apply automation). A top-tier version must default to *assistive drafting* and require manual execution/confirmation. 

### Revised top roadmap priorities

**Priority zero (low effort, high leverage):**
- Add LICENSE + SECURITY.md + LEGAL/ToS disclaimer patterned after mature comparables. 
