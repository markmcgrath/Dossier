...
- `skill/references/mode12-batch-pipeline.md`: lightweight batch scoring step cited the wrong dimension numbers — said "Dimensions 1 (Role Match), 2 (Company Quality), 3 (Compensation), and 10 (Strategic Value)" but the canonical schema in `mode1-offer-evaluator.md` / `scoring-guide.md` puts Company Quality at Dimension 6 and Compensation at Dimension 4 (Dimensions 2 and 3 are Skills & Experience and Seniority). The labels were correct shortcuts; the numbers were off-by-pointer. Mode 12 now reads "Dimensions 1 (Role Match), 6 (Company Quality), 4 (Compensation), and 10 (Strategic Value)" so a batch scoring run hits the dimensions the intent named.
- `skill/references/mode7-salary-negotiation.md`: offer-comparison save path was `Dossier/offer-comparison-[YYYY-MM-DD].md` (vault root, inconsistent with every other artifact type). Now `negotiation/offer-comparison-[YYYY-MM-DD].md` so the file sits next to its sibling `negotiation-[slug]-[date].md` artifacts and matches the subfolder-per-type convention documented in `file-conventions.md`.
- `CONTRIBUTING.md` §Skill Development Workflow: repack instructions cited `python -m scripts.package_skill /path/to/skill/ /tmp/out/`, which is not a module that exists in this repo. The real packager is `.github/scripts/build_skill.sh`. Replaced the bogus command with the in-repo invocation (`bash .github/scripts/build_skill.sh && cp dist/dossier-*.skill dossier.skill`); a contributor following the prior text would have hit `ModuleNotFoundError`. Removed the PR-checklist line referencing a non-existent nested `open-source/` subdirectory — this repo *is* the open-source copy, so the checkbox could never be satisfied.
- `examples/example-eval.md`: `model` field said `Claude 3.5 Sonnet` (display name from an older Claude generation) while the schema example in `file-conventions.md` and all three `examples/golden/` artifacts use `claude-sonnet-4-6` (canonical model ID). Updated for consistency. Also appended the required `## Related` section with wikilinks to the companion `example-outreach`, `example-cover-letter`, and `example-prep` showpieces — `file-conventions.md` mandates this section on every eval that has related artifacts, and the showpiece eval was the only one missing it.
- `examples/example-prep.md`: frontmatter was missing the `interview_date:` and `interviewers:` fields that `file-conventions.md` requires on every `type: prep` file. Added both with plausible-fictional values consistent with the rest of the Cipher Analytics narrative.
- `HARDENING.md`: §1 branch-protection ruleset and §7 CI surface description both listed only four required status checks (`pii-scan`, `test (3.11)`, `test (3.12)`, `changelog-check`) while `ci.yml` defines six jobs. The `skill-parity` job (added in PR #50, "gate PRs on skill-parity") and the `conventional-commits` job (PR #42, "enforcement") were silently absent from both lists. Both sections now enumerate all six checks with their PR-only vs. push-also gating noted. Stale "121 passing, 3 skipped" test surface count in §7 corrected to "176 passing, 3 skipped".

### Documentation

- `features/plan/README.md`: was the original "Dossier v2" streams index for plans 01–07 with no mention of any of plans 08–21 that followed. Added a "Subsequent plans" navigation table linking every plan doc in the folder. Header status updated from "Ready for implementation" (Streams A–E shipped in April–May 2026) to a note pointing at the new section. Plan 11 still has no plan doc; that gap is now documented in the table footer.
- `features/plan/19-routing-eval-harness.md`: post-hoc stub for the routing-eval harness work that shipped in PR #43 (commit `b92a31d`) without a corresponding plan document. Stub follows Plan 21's format — it states what shipped, cross-references PR #43 for the design rationale (especially the mid-PR redesign that dropped the CI publication-gate posture), and explicitly disclaims coverage of future improvements. Closes the plan-numbering gap.
- `features/plan/17-eval-schema-data-hygiene.md` validation checklist: six items were left unchecked even though the plan body and CHANGELOG show the work shipped. Boxes ticked; the one item about "open-source mirror repack" rewritten to acknowledge this repo *is* the open-source copy (no second repack target).
- `features/plan/20-v1-3-1-and-phase-2-foundations.md` status header: was `**Status:** Proposed` despite all three phases (v1.3.1 patch, Target Radar brief stub, `examples/golden/`) having shipped on 2026-05-11. Now `**Status:** Shipped` with a `**Closed:**` line listing the PRs that closed each phase.
